...
const witnessMode = { type: "string", enum: ["none", "summary", "full_required"], default: "none" };
...
export const TOOL_DEFINITIONS = deepFreezeContract([
  {
    ...
    outputSchema: MCP_OUTPUT_SCHEMAS.inspect,
    annotations
  },
  {
    name: "text_normalize", title: "Normalize Unicode text",
    description: "Apply one named Unicode normalization form and report equivalence without mutating the input. Optionally return a bounded summary or complete decomposition/order/composition witness; complete witnesses never truncate.",
    operation: "normalize",
    inputSchema: {
      type: "object", additionalProperties: false, required: ["text", "form"],
      properties: { text: text(), form: { type: "string", enum: ["NFC", "NFD", "NFKC", "NFKD"] }, witnessMode }
    },
    outputSchema: MCP_OUTPUT_SCHEMAS.normalize,
    annotations
  },
  {
    ...
    outputSchema: MCP_OUTPUT_SCHEMAS.compare,
    annotations
  },
  {
    name: "text_transcode", title: "Transcode text or bytes",
    description: "Encode text or strictly decode and re-encode bytes in the closed UTF-8/UTF-16LE set; replacement is opt-in and reported. Optionally return a bounded summary or complete source-to-output witness; complete witnesses never truncate. Select one byte representation to avoid duplicated payload copies.",
    operation: "transcode",
    ...
    outputSchema: MCP_OUTPUT_SCHEMAS.transcode,
    annotations
  },
  {
    ...
    outputSchema: MCP_OUTPUT_SCHEMAS.security,
    annotations
  },
  {
    name: "text_explain_difference", title: "Explain why two strings differ",
    description: "Compare exact representation, four normalization forms, NFKC_Casefold, first differing code point/grapheme, coordinates, invisibles, line endings, collation, and identifier confusability. Optional witnesses include deterministic complete code-point and grapheme alignments; complete witnesses never truncate and runtime-bound facts stay labeled.",
    operation: "explain_difference",
    inputSchema: pairInput({ confusableDirection: direction, detailLimit, witnessMode }, ["confusableDirection"]),
    outputSchema: MCP_OUTPUT_SCHEMAS.explain_difference,
    annotations
  },
  {
  ...
  },
  {
    ...
    description: "Apply UTS #46 domain processing or one named RFC 8265 PRECIS profile, separately from ordinary normalization. Optionally return a bounded summary or complete named-stage witness; complete witnesses never truncate.",
    operation: "protocol_profile",
    inputSchema: { type: "object", oneOf: [
      {
        type: "object", additionalProperties: false, required: ["profile", "action", "text", "options"],
        properties: { profile: { const: "uts46_domain" }, action: { const: "to_ascii" }, text: text(), options: uts46Options(true), witnessMode }
      },
      {
        type: "object", additionalProperties: false, required: ["profile", "action", "text", "options"],
        properties: { profile: { const: "uts46_domain" }, action: { const: "to_unicode" }, text: text(), options: uts46Options(false), witnessMode }
      },
      {
        type: "object", additionalProperties: false, required: ["profile", "action", "text"],
        properties: { profile: precisProfile, action: { const: "enforce" }, text: text(), witnessMode }
      },
      {
        type: "object", additionalProperties: false, required: ["profile", "action", "text", "comparison"],
        properties: { profile: precisProfile, action: { const: "compare" }, text: text(), comparison: text(), witnessMode }
      }
    ] },
    outputSchema: MCP_OUTPUT_SCHEMAS.protocol_profile,
    annotations
  }
]);
